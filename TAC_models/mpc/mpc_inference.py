...
def get_completions(main_trie, suffix_trie, prefix, completions_file, k_completions=10, suffix_context=2):
    res = []
...
def init(args):
    ...
    with open(args.output_file, 'w', encoding='utf-8') as dfile:
        with tqdm(total=total_count) as pbar:
            for is_valid, input_text, output_text in load_text_stream(args.input_file, args.data_limit, gt=1, context=args.context):
                pbar.update(1)
                if not is_valid:
                    break
                ...
                completions = get_completions(query_completion, suffix_completion, prefix, 
                                            args.completions_file, args.k_completions, args.suffix_context_words)

                dfile.write(json.dumps({"prefix": prefix, "gt": output_text, "completions": completions}) + "\n")
                if completions != []:
                    inference_out.append(f"{prefix}\t{output_text}\t{completions[0][0][len(prefix):]}\t{completions[0][1]}\t1\n")
                else:
                    inference_out.append(f"{prefix}\t{output_text}\t\tMT:0\t1\n")
                inference_count+=1
    
    if (args.context == 0):
        with open(args.completions_file, 'w') as f:
            f.writelines(inference_out)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--completions_file',
                        type=str,
                        default=None,
                        help='path to location to save the trie object.')
