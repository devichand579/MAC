...
def init(args):
    ...
    with open(args.output_file, 'w', encoding='utf-8') as dfile:
        with tqdm(total=total_count) as pbar:
            for is_valid, input_text, output_text in load_text_stream(args.input_file, args.data_limit, gt=1):
                pbar.update(1)
                if not is_valid:
                    break
    ...
    total_time = end_time - start_time  # Calculate total inference time
    average_inference_time = total_time / inference_count if inference_count > 0 else 0  # Calculate average inference time
    print("Total inference: ", inference_count)
    print("Total time: {:.2f} seconds".format(total_time))
    print("Average inference time per data point: {:.4f} seconds".format(average_inference_time))
