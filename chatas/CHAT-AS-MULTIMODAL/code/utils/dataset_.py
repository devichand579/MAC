...
class Utterance:
    ...
    def format_utterance(self) -> str:
        ...
        return "".join(["[" + image + "]" for image in self.images] + [self.text])
...
def update_intersection():
    ...
    with open("data/ImageChat/intersection.json", "w") as f:
...
def update_train_contextual_txt(train_data: DialogData, filename: str):
    """
    Creates a training file where each line contains the context utterances 
    joined by <EOU> tokens, followed by a tab and the response.
    Format: context_line1<space><EOU><space>context_line2<space><EOU><space>...<space><EOU><space><tab>response
    """
    context_response_pairs = []
    for dialog, _ in tqdm(train_data, desc="Formatting train contexts"):
        context = dialog.context
        response = dialog.response.text.lower()
        
        if context:
            context_str = " <EOU> ".join([utt.text.lower() for utt in context])
            formatted_line = f"{context_str} <EOU> \t{response}"
        else:
            formatted_line = f"<EOU> \t{response}"
            
        context_response_pairs.append(formatted_line)
    
    with open(f"data/ImageChat/{filename}", "w") as f:
        f.write("\n".join(context_response_pairs))

def update_test_formatted_contextual(test_data: DialogData):
    """
    Creates a test file where each line contains the context utterances 
    joined by <EOU> tokens, followed by tab-separated prefix and suffix.
    Format: context_line1<space><EOU><space>context_line2<space><EOU><space>...<space><EOU><space>prefix<tab>suffix
    """
    context_prefix_suffix = []
    
    for dialog, suffix in tqdm(test_data, desc="Formatting test contexts"):
        context = dialog.context
        prefix = dialog.response.text.lower()
        suffix = str(suffix).lower()
        
        if context:
            context_str = " <EOU> ".join([utt.text.lower() for utt in context])
            formatted_line = f"{context_str} <EOU> {prefix}\t{suffix}"
        else:
            formatted_line = f"<EOU> {prefix}\t{suffix}"
            
        context_prefix_suffix.append(formatted_line)
    
    with open("data/ImageChat/test_formatted_contextual.txt", "w") as f:
        f.write("\n".join(context_prefix_suffix))
    ...
    for dialog, suffix in test_data:
        ids.append(dialog.idx)
        if dialog.context:
            context_str = " <EOU> ".join([utt.text.lower() for utt in dialog.context])
            contexts.append(f"{context_str} <EOU> ")
        else:
            contexts.append("<EOU> ")
        prefixes.append(dialog.response.text.lower())
        suffixes.append(str(suffix).lower())
    ...
    df.to_csv("data/ImageChat/test_formatted_contextual.csv", index=False)
...
if __name__ == "__main__":
    ...
    print(len(test_data))
    update_train_contextual_txt(train_data, 'train_contextual.txt')
    update_test_formatted_contextual(test_data)
    # # exit(0)
